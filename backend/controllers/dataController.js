...
const getData = async (req, res) => {
  try {
    ...
    const mbtiData = await data.find({}).toArray(); // 빈 객체를 전달하면 해당 컬렉션에 있는 애들 다 찾아옴. toArray로 전체 데이터를 배열 형태로 뽑아서 mbti라는 변수에 넣어줌
    res.status(200).json(mbtiData); // mbtiData 를 Json 형태로 통으로 보내줌
  } catch (err) {
    console.error(err);
    res.status(500).json('데이터 삽입 실패, 알 수 없는 문제 발생');
  }
